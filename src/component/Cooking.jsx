...
const Cooking = ({ item, handleCooked }) => {
  const { order_title, quantity, special_instruction } = item;
  return (
    <div
    ...
    >
      {/* Title */}
      <h3 className="text-xl font-bold text-primary mb-2">{order_title}</h3>
      ...
      {/* Special Instruction */}
      <p className="text-xs text-gray-600 mt-2 italic">{special_instruction}</p>

      {/* Static Button */}
      <button
        onClick={() => handleCooked(item)}
